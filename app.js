(function(){
  ...
  const catalog = Array.isArray(window.ABC_CATALOG) ? window.ABC_CATALOG : [];
  ...
  function smartOrder(ms){
    const shuffled=seededShuffle(catalog.filter(x=>x.cleared&&x.videoId),`ABC:${weekKey(ms)}:${dateKey(ms)}`);
    const result=[];
  ...
  function renderPremium(){
    const targets=Array.isArray(window.ABC_PREMIUM_TARGETS)?window.ABC_PREMIUM_TARGETS:[];
  ...
  function loadProgram(block,elapsed){
    ...
    if(sourceEnded){
      showCard("ABC",program.title,"The complete source ended. Next scheduled program starts soon.");
      return;
    }
  ...
  function tick(){
    ...
    els.mode.textContent=mode==="live"?"LIVE ABC":"TIME SHIFTED";
  ...
  function localShareCredit(reference){
    const attemptId=`abc-share-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`;
    ...
    profile.ledger.push({id:`tx-${attemptId}`,type:awarded?"share_reward":"share_credit",amount:awarded,balance:profile.tokens,pendingShareCredits:profile.pendingShareCredits,reason:awarded?"Share reward: 10 completed shares":`Confirmed ABC share ${profile.pendingShareCredits}/10`,referenceId:attemptId,createdAt:Date.now()});
  ...
  async function share(){
    const block=currentBlock(Date.now()), title=block?block.program.title:"ABC";
  ...
  window.onYouTubeIframeAPIReady=function(){
    player=new YT.Player("player",{
      ...
      events:{
        ...
        onError:()=>{const block=currentBlock(activeMs());showCard("SOURCE UNAVAILABLE",block?block.program.title:"ABC","This official source cannot play in the embedded player right now. The station will continue with the next scheduled program.");}
      }
    });
  };
